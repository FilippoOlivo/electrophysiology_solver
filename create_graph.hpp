...
public:
                ...
                std::string filename)
  {
    std::vector<double> snapshot_vector(local_size, 0);
      ...
      cnpy::npy_save(filename2, snapshot_vector.data(), {snapshot_vector.size()}, "w");
...
private:
  ...
  extract_local_points(DoFHandler<dim> &dof_handler, MappingQ<dim> &mapping)
  {
    ...
    std::vector<std::vector<double>> local_points(local_size, std::vector<double>(3));
    for (auto idx : locally_owned_dofs)
      {
        for (int j =0; i<dim; i++)
  ...
  save_points(std::vector<std::vector<T>> &points, std::string base_filename)
  {
    ...
    if constexpr (std::is_same<T, int>::value) {
        tensor = torch::zeros({static_cast<int64_t>(points.size()), static_cast<int64_t>(points[0].size())}, torch::kInt);
    } else {
        tensor = torch::zeros({static_cast<int64_t>(points.size()), static_cast<int64_t>(points[0].size())}, torch::kDouble);
    }
